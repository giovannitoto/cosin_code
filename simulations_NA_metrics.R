...
for(s in s_list) {
  ...
  # filename
  table_filename <- paste0("table_metrics_nometa_NA_s", s)
  # compute metrics
  table_metrics <- foreach(i = 1:nrow(sim_list), .combine = 'rbind', .inorder = FALSE) %dopar% {
    ...
    for (it in 1:sp) {
      ...
      mae_Y <- mae_Y + mean(abs(Y_it[is.na(sim$Y_na)] - sim$Y[is.na(sim$Y_na)])) / sp
      mad_Y <- mad_Y + mean(abs(Y_it[is.na(sim$Y_na)] - mean(Y_it[is.na(sim$Y_na)]))) / sp
    }
    # Benchmark
...
for(s in s_list) {
  ...
  sim_list <- expand.grid(r = R_list, na_prop = na_prop_list, sigma = sigma_list, np = np_list)
  # filename
  table_filename <- paste0("table_metrics_meta_NA_s", s)
  # compute metrics
  table_metrics <- foreach(i = 1:nrow(sim_list), .combine = 'rbind', .inorder = FALSE) %dopar% {
    # ------------------------------------------------------------------------ #
    # get n, p, sigma, scenario (s), replication (r), seed
