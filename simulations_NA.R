...
get_output <- function(lambda, eta, wB, hyper, na_prop) {
  ...
  # sample y_na until no rows and columns have only NA values
  for(ttry in 1:100) {
    y_na <- y
    y_na[sample(1:(n*p), round(hyper$na_prop * n * p))] <- NA
    checkNA <- any(apply(y_na, 1, function(x) all(is.na(x))), apply(y_na, 2, function(x) all(is.na(x))))
    if(!checkNA) break
  }
  # output
...
# filter combinations
#sim_list <- sim_list[sapply(sim_list$np, function(np) np[[2]]) == 1000, ]
...
foreach(i = 1:nrow(sim_list), .combine = 'rbind', .inorder = FALSE) %dopar% {
  ...
  if(s == 1) {
    ...
    wB <- data.frame("wB1" = as.factor(wB))
  } else if(s == 2) {
    # lambda
    l1 <- rnorm(p, 0, sigma_lambda)
  ...
  # get output
  sim <- get_output(lambda, eta, wB, hyper)
